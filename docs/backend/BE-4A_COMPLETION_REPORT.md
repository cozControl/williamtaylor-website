...
The original BE-4A implementation incorrectly introduced pages.*, roles.assign, and roles.revoke instead of the authorized nine-permission foundation. BE-4A.1 corrects the registry and role bundles through a preview-first audited alignment, uses roles.manage for both mutation methods, and adds admin.access for the future shell boundary. See BE-4A-1_PERMISSION_ALIGNMENT_REPORT.md for current evidence. No CMS permission remains.
