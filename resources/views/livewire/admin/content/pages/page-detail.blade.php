<div class="cms-page-detail" x-on:open-page-preview.window="window.open($event.detail.url, '_blank', 'noopener')">
    ...
    <section class="admin-panel cms-page-summary">
        ...
        <dl class="admin-definition-grid">
            ...
            <div><dt>Current draft revision</dt><dd>{{ $this->page->currentDraftRevision->revision_number }}</dd></div>
            <div><dt>Candidate revision</dt><dd>{{ $publication?->candidateRevision?->revision_number ?? 'None' }}</dd></div>
            <div><dt>Designated published revision</dt><dd>{{ $publication?->currentPublicRevision?->revision_number ?? 'None' }}</dd></div>
        </dl>
        @if($publication?->current_public_revision_id)<p class="admin-notice">Public storefront projection is not active yet.</p>@endif
        @if($publication?->candidate_revision_id && $publication->candidate_revision_id !== $this->page->current_draft_revision_id)<p class="admin-warning">A newer unsubmitted draft exists. It does not change the governed candidate.</p>@endif
        <div class="admin-page-actions">
            ...
            @if($publication?->candidateRevision) @can(\App\Domain\Identity\Support\PermissionRegistry::PAGES_PREVIEW)<button class="admin-secondary-button" type="button" wire:click="preview('{{ $publication->candidate_revision_id }}')">Preview candidate</button>@endcan @endif
            @if($publication?->currentPublicRevision) @can(\App\Domain\Identity\Support\PermissionRegistry::PAGES_PREVIEW)<button class="admin-secondary-button" type="button" wire:click="preview('{{ $publication->current_public_revision_id }}')">Preview designated published</button>@endcan @endif
        </div>
    </section>
    ...
    <section class="admin-panel" aria-labelledby="workflow-actions-title"><h2 id="workflow-actions-title">Workflow actions</h2>
        ...
        @if($publication?->candidate_revision_id)<label class="admin-confirmation"><input type="checkbox" wire:model="confirmSupersession"> Confirm candidate supersession when submitting the current draft</label>@endif
        <div class="admin-page-actions">
            ...
            @if($candidateState === 'in_review') @can(\App\Domain\Identity\Support\PermissionRegistry::PAGES_APPROVE)<button class="admin-primary-button" type="button" wire:click="approve">Approve candidate</button>@endcan @endif
            @if($candidateState === 'approved') @can(\App\Domain\Identity\Support\PermissionRegistry::PAGES_PUBLISH)<button class="admin-primary-button" type="button" wire:click="publish">Designate published now</button>@endcan @endif
        </div>
    ...
    <section class="admin-panel"><h2>Elevated lifecycle actions</h2><p>Archive never changes publication state implicitly. Scheduled Pages must be cancelled first; designated published Pages must be unpublished first.</p><div class="admin-field"><label for="page-lifecycle-reason">Required reason</label><textarea id="page-lifecycle-reason" wire:model="reason" maxlength="2000"></textarea></div>
        <div class="admin-page-actions">
            ...
            @if($publication?->current_public_revision_id) @can(\App\Domain\Identity\Support\PermissionRegistry::PAGES_UNPUBLISH)<button class="admin-danger-button" wire:click="unpublish">Unpublish designated revision</button>@endcan @endif
