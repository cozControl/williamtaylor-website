<x-admin.layout title="New Product" description="Create a complete storefront Product and its sellable Variants." eyebrow="Catalogue" :breadcrumbs="['Products' => route('admin.products.index'), 'New Product' => null]">
    ...
    <form method="POST" action="{{ route('admin.products.store') }}">
        ...
        <section class="admin-panel admin-form-grid">
            ...
            <x-admin.field label="Primary Category" for="product-primary-category" :error="$errors->first('primary_category_id')"><select id="product-primary-category" name="primary_category_id" @if($errors->has('primary_category_id')) aria-invalid="true" aria-describedby="product-primary-category-error" @endif><option value="">Choose Category</option>@foreach($categories as $category)<option value="{{ $category->id }}" @selected(old('primary_category_id') === $category->id)>{{ $category->parent?->name ? $category->parent->name.' → ' : '' }}{{ $category->name }}</option>@endforeach</select></x-admin.field>
            <fieldset><legend>Additional Categories</legend>@foreach($categories as $category)<label><input type="checkbox" name="category_ids[]" value="{{ $category->id }}" @checked(in_array($category->id, old('category_ids', []), true))> {{ $category->name }}</label>@endforeach</fieldset>
        </section>
